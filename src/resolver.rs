...
use crate::value::all_native_ids;
...
impl Resolver {
    pub fn new() -> Resolver {
        ...
        for native_id in all_native_ids() {
            let result =
                resolver.define(&native_id.to_string(), &SourceLoc::default());
            if let Err(error) = result {
                panic!(
                    "Resolver::new(): defining native function failed: {}",
                    error.message
                );
            }
        }
    ...
    ) -> Result<ResolvedCode, ParseError> {
    ...
    fn resolve_expression(
        ...
        match expression {
            ...
            Expr::Super(super_dist_cell, _, loc) => {
                ...
                let super_var_loc =
                    self.resolve_local_variable("super", loc)?;
    ...
    fn resolve_function(
        ...
        if result.is_ok() {
            for body_statement in &mut fun_def.body {
    ...
    fn forward_reserve_global_var(
        &mut self,
        identifier: &str,
        loc: &SourceLoc,
    ) -> Result<usize, ParseErrorCause> {
...
fn ensure_scope_index_limit(
    new_scope_len: usize,
    identifier: &str,
    loc: &SourceLoc,
) -> Result<(), ParseErrorCause> {
    if new_scope_len > VAR_LOC_MAX_DISTANCE_USIZE {
